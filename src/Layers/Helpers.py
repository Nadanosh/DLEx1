...
def gradient_check(layers, input_tensor, label_tensor):
    ...
    for i in range(input_tensor.shape[1]):
        for j in range(input_tensor.shape[0]):
            ...
            plus_epsilon[j, i] += epsilon
            minus_epsilon = input_tensor.copy()
            minus_epsilon[j, i] -= epsilon
            ...
            analytical_derivative = error_tensor[j, i]
            ...
            if normalizing_constant < 1e-15:
                ...
                difference[j, i] = np.abs(analytical_derivative - numerical_derivative) / normalizing_constant
    return difference
...
def shuffle_data(input_tensor, label_tensor):
    index_shuffling = [i for i in range(input_tensor.shape[1])]
    ...
    return np.transpose(np.array(shuffled_input)), np.transpose(np.array(shuffled_labels))
...
class RandomData:
    ...
    def forward(self):
        input_tensor = np.random.random([self.input_size, self.batch_size])
...
class IrisData:
    def __init__(self):
        self.data = load_iris()
        self.label_tensor = np.zeros([3, 150])
        for i in range(150):
            self.label_tensor[self.data.target[i], i] = 1

        self.input_tensor, self.label_tensor = shuffle_data(np.transpose(np.array(self.data.data)), self.label_tensor)
