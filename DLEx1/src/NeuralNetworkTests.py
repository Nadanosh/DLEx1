...
class TestFullyConnected(unittest.TestCase):
    def setUp(self):
        ...
        self.input_size = 4
        self.output_size = 3
        self.input_tensor = np.random.rand(self.input_size, self.batch_size)

        self.categories = 4
        self.label_tensor = np.zeros([self.categories, self.batch_size])
        for i in range(self.batch_size):
            self.label_tensor[np.random.randint(0, self.categories), i] = 1
    ...
    def test_forward_size(self):
        ...
        self.assertEqual(output_tensor.shape[0], self.output_size)
        self.assertEqual(output_tensor.shape[1], self.batch_size)

    def test_backward_size(self):
        layer = FullyConnected.FullyConnected(self.input_size, self.output_size)
        output_tensor = layer.forward(self.input_tensor)
        error_tensor = layer.backward(output_tensor)
        self.assertEqual(error_tensor.shape[0], self.input_size)
        self.assertEqual(error_tensor.shape[1], self.batch_size)
    ...
    def test_update(self):
        ...
        for _ in range(10):
            output_tensor = layer.forward(self.input_tensor)
            error_tensor = np.zeros([self.output_size, self.batch_size])
    ...
    def test_gradient(self):
        input_tensor = np.abs(np.random.random((self.input_size, self.batch_size)))
        layers = list()
...
class TestReLU(unittest.TestCase):
    def setUp(self):
        ...
        self.input_tensor = np.ones([self.input_size, self.batch_size])
        self.input_tensor[:, 0:self.half_batch_size] -= 2

    def test_forward(self):
        expected_tensor = np.zeros([self.input_size, self.batch_size])
...
class TestSoftMax(unittest.TestCase):
    ...
    def test_predict(self):
        ...
        np.testing.assert_almost_equal(expected_values, prediction)
...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_iris_data(self):
        ...
        one_hot_vector = results / maximum[None, :]
        one_hot_vector[one_hot_vector != 1] = 0
        
        correct = 0
        wrong = 0
        for column_results, column_labels in zip(one_hot_vector.T, labels.T):
